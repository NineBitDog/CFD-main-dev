...
void main_setup() { // Custom; required extensions in defines.hpp: FP16S, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS or GRAPHICS
    ...
    const float lbm_u = INLET_VELOCITY;
    const ulong lbm_T = 108000ull;
    LBM lbm(lbm_N, 2u, 2u, 2u, units.nu_from_Re(lbm_Re, (float)lbm_N.x, lbm_u)); // run on 2x2x2 domains
    // ###################################################################################### define geometry ######################################################################################
    ...
    parallel_for(lbm.get_N(), [&](ulong n) {
        ...
        // Start the entire fluid at rest. The OpenCL stream-collide kernel ramps
        ...
        if(lbm.flags[n]!=TYPE_S) {
            // Wind-tunnel boundaries: equilibrium inlet/outlet only in X.
            // Y/Z remain periodic so there are no artificial reflecting walls.
            if(x==0u || x==Nx-1u) {
                lbm.flags[n] = (lbm.flags[n]&~TYPE_E)|TYPE_E;
    ...
    while(lbm.get_t()<=lbm_T && running) { // main simulation loop
        ...
        if(key_9) {
            ...
            string manual_path = R"(C:/Users/21bry/Downloads/CFD-main-dev/FluidX3D-master/bin/export/)";
