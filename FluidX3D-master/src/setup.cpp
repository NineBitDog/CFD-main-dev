...
void main_setup() { // Custom; required extensions in defines.hpp: FP16S, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS or GRAPHICS
    ...
    LBM lbm(lbm_N, 1u, 1u, 1u, units.nu_from_Re(lbm_Re, (float)lbm_N.x, lbm_u)); // run on one GPU/domain: one LBM buffer
    // ###################################################################################### define geometry ######################################################################################
    ...
    Clock clock;
    lbm.voxelize_stl(get_exe_path()+"../stl/sim_geometry_1787167744.stl", center, rotation, size);
