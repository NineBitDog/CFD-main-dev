...
# Patch the native graphics_streamline kernel.  The generated kernel.cpp has
# changed its surrounding comments over time, so locate the actual current
# streamline body instead of depending on one stale comment marker.
...
# Filter the native colored velocity volume in ray_grid_traverse_sum().
...
# Patch the colored velocity volume once. If it was already patched, leave it.
if "if(speed_disturbance>=GRAPHICS_STREAMLINE_SPEED_THRESHOLD || direction_disturbance>=GRAPHICS_STREAMLINE_DIRECTION_THRESHOLD)" not in s:
